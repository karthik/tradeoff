...
run_vdm_jg <- function(m, sJ, sA, Fec, juvshape) {
    # message("\n entering rum_vdm_jg \n")
    # message(sprintf("m %s, sA %s, sJ %s, Fec %s, juvshape %s \n",m,sA,sJ,Fec,juvshape))
    # message("\n")
    ...
    cv <- sqrt(juvshape)/juvshape
    # message(sprintf("Shape: %s, scale:%s CV:%s\n\n", juvshape, juvscale, cv))
    message(sprintf("\n CV:%s\n\n",  cv))
    if(round(meanjuv)==99 && round(juvscale)==33)
        return(NULL)
...
# -----------------------------------------
run_vdm_corr <- function(m, sJ, sA, Fec, corr) {
...
theme_complete_bw <- function(base_size = 12) {
    structure(list(axis.line = theme_blank(), axis.text.x = theme_text(size = base_size *
        ...
        axis.title.x = theme_text(size = base_size, vjust = 0.5), axis.title.y = theme_text(size = base_size,
            angle = 90, vjust = 0.5), axis.ticks.length = unit(0.15, "cm"), axis.ticks.margin = unit(0.1,
